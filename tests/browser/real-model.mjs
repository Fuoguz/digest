...
import { cases as previousCases } from '../fixtures/real-model-cases.mjs';
const cases=process.env.DIGEST_ACCEPTANCE==='20260922'?(await import('../fixtures/pilot-acceptance-20260922.mjs')).cases:previousCases;
...
const out=process.env.DIGEST_ACCEPTANCE==='20260922'?'qa-artifacts/pilot-20260922/real-model':'qa-artifacts/real-model'; await mkdir(out,{recursive:true});
...
try {
 ...
 for(const c of cases.filter(c=>!process.env.DIGEST_CASE || c.id===process.env.DIGEST_CASE)){
  ...
  try{const snapshot=await documentSnapshot(doc);let output;if(c.mode){output=await new DigestAIService(transport).analyze(snapshot,{requestId:'acceptance-'+c.id});}else{const task={id:'task-'+c.id,prompt:c.prompt,rubric:c.rubric,documentIds:[doc.id]},course={id:'course',title:'公开材料课程验收',description:'只按所给节选评价，允许有依据的不同观点。'};output=await generateFeedback(await prepareFeedbackContext(task,course,[doc],c.answer),transport,'feedback-'+c.id);}
   const evidenceChecks=(output.anchors||[]).map(a=>({id:a.id,status:a.validationStatus,version:a.sourceRevision===snapshot.sourceRevision,offsets:a.validationStatus!=='matched'||snapshot.source.slice(a.startOffset,a.endOffset)===a.quote,paragraph:a.validationStatus!=='matched'||snapshot.paragraphs[a.paragraphIndex]?.text.slice(a.paragraphStartOffset,a.paragraphEndOffset)===a.quote}));
   results.push({id:c.id,expected:c.expected,checklist:c.checklist,sourceChars:c.source.length,sourceParagraphs:snapshot.paragraphs.length,sourceRevision:snapshot.sourceRevision,evidenceChecks,elapsedMs:Date.now()-start,output});console.log(c.id,'STRUCTURE PASS');
