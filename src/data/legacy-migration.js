...
export function extractLegacyNodes(snapshot) {
  ...
  return nodes.filter((node) => {
    ...
    const signature = String(node.id || '') + '|' + String(node.title || node.label || '') + '|' + String(node.createdAt || '');
...
export function legacyNodeToDocument(node, index = 0) {
  ...
  const insight = String(node.insight || node.mainPoint || node.content || '').trim();
...
export async function runLegacyMigration({ storage = globalThis.localStorage, db } = {}) {
  ...
  if (!snapshot) {
    const result = { status: 'completed', migrated: 0, backupCreated: false, completedAt: new Date().toISOString() };
    await putSetting(MIGRATION_MARKER, result, db);
    return result;
  }
  try {
    const backupJSON = JSON.stringify(snapshot, null, 2);
    try { storage?.setItem?.(LEGACY_BACKUP_KEY, backupJSON); } catch { /* IndexedDB backup remains available. */ }
    ...
    const result = { status: 'completed', migrated: documents.length, backupCreated: true, completedAt: new Date().toISOString() };
    await putSetting(MIGRATION_MARKER, result, db);
    return result;
  } catch (error) {
    const result = { status: 'failed', migrated: 0, backupCreated: Boolean(storage?.getItem?.(LEGACY_BACKUP_KEY)), message: error.message };
    try { await putSetting(MIGRATION_MARKER, result, db); } catch { /* A later launch can retry. */ }
    return result;
  }
}
