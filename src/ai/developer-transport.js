...
export function saveDeveloperConfig(config, storage = globalThis.localStorage) {
  ...
  if (
    ...
    throw new AnalysisError(
      "configuration",
      tr("服务地址必须使用 HTTPS，本地开发可使用 localhost HTTP。"),
    );
  }
  if (
    endpoint.username ||
    ...
    throw new AnalysisError(
      "configuration",
      tr("请填写不含凭据或查询参数的服务地址。"),
    );
  if (!config.model.trim())
    throw new AnalysisError("configuration", tr("请填写模型名称。"));
  storage.setItem(
    "digest:developer-ai",
...
export class DeveloperTransport {
  ...
  async request(messages, { signal } = {}) {
    ...
    if (!endpoint || !model || !key)
      throw new AnalysisError(
        "no_configuration",
        tr("尚未配置 Developer AI Service。原文阅读不受影响。"),
      );
    ...
    try {
      ...
      if (typeof text !== "string" || !text.trim())
        throw new AnalysisError("empty_result", tr("AI 服务返回空结果。"));
      return text;
    } catch (error) {
      if (timedOut)
        throw new AnalysisError(
          "timeout",
          tr("分析超时。可以重试，原有结果仍然保留。"),
        );
      if (signal?.aborted)
      ...
      throw new AnalysisError(
        "network_error",
        tr("无法连接 AI 服务，请检查网络、服务地址及浏览器 CORS 支持。"),
      );
    } finally {
      clearTimeout(timer);
