...
export const MODE_SECTIONS = Object.freeze({
  ...
  academic_paper: {
    researchQuestion: tr("研究问题"),
    method: tr("研究方法"),
    findings: tr("主要发现"),
    evidence: tr("研究证据"),
    limitations: tr("研究局限"),
    reusableInsights: tr("可复用洞见"),
  },
  legal_case: {
    facts: tr("案件事实"),
    legalIssues: tr("法律争点"),
  ...
  policy_document: {
    background: tr("政策背景"),
    targetAudience: tr("适用对象"),
    objectives: tr("政策目标"),
    measures: tr("主要措施"),
    responsibleActors: tr("责任主体"),
    timeline: tr("实施时间"),
    implications: tr("影响与启示"),
  },
});
export const CLAIM_KINDS = {
  source_statement: tr("资料陈述"),
  inference: tr("AI 推论"),
  reusable_insight: tr("可复用洞见"),
};
  ...
  for (const name of Object.keys(MODE_SECTIONS[mode])) {
    ...
    const items = section.items.map((claim) => {
      if (
        ...
        fail(tr("Claim 内容、类型或 id 无效。"));
      if (!Array.isArray(claim.evidence) || claim.evidence.length > 12)
        fail(tr("Claim evidence 必须是建议数组。"));
      ids.add(claim.id);
      count++;
      ...
      return {
        ...
        evidence: claim.evidence.map((proposal) => ({
          ...
          ...(typeof proposal?.quoteId === "string"
...
export function parseReadingOutput(text, mode) {
  ...
  try {
    ...
    throw new AnalysisError("invalid_json", tr("AI 未返回有效 JSON。"));
  }
