...
export async function mountGraph(container, db, isCurrent) {
  ...
  const root = page(
    tr("知识之间，有迹可循"),
    tr("Knowledge network"),
    tr("每个点由你沉淀，每条线都有理由。孤立的知识也值得保留。"),
  );
  const feedback = el("p"),
  ...
  function inspect(unit) {
    ...
    detail.replaceChildren(
      el("p", "page-kicker", tr("KNOWLEDGE / 知识点")),
      el("h2", "", unit.label),
    ...
    if (!unit.evidenceIds.length)
      detail.append(
        el("p", "reader-muted", tr("暂无可靠原文依据，请核对来源判断。")),
      );
    ...
    actions.append(
      button(
        tr("编辑名称"),
        () =>
          editName(tr("知识点名称"), unit.label, (name) =>
    ...
    if (!connected.length)
      detail.append(el("p", "reader-muted", tr("尚无确认关系。")));
  }
  function inspectRelation(r) {
    ...
    detail.replaceChildren(
      el("p", "page-kicker", tr("RELATION / 用户确认")),
    ...
    for (const id of [r.sourceKnowledgeUnitId, r.targetKnowledgeUnitId]) {
      ...
      if (u)
        detail.append(
          ...
          sourceLink(u, tr("核对 ") + u.label + tr(" 的来源")),
        );
    }
    detail.append(
      button(
  ...
  async function suggest() {
    ...
    if (!useProxy() && (!config.key || !config.model || !config.endpoint)) {
      openAIConfiguration();
      message(feedback, tr("配置后再次点击建议关系。"));
      return;
    }
    const selected = filteredUnits().filter((u) => !u.stale);
    if (selected.length < 2) {
      message(feedback, tr("至少需要两个有效知识点。"));
      return;
    }
    if (selected.length > 40) {
      message(feedback, tr("请先筛选至 40 个以内，再建议关系。"));
      return;
    }
    controller = new AbortController();
    const signal = controller.signal;
    message(feedback, tr("正在比较知识点；没有可靠关系时会返回空结果。"));
    renderTools();
    try {
      ...
      message(
        feedback,
        count
          ? tr("新增 ") + count + tr(" 条建议，请核查后确认。")
          : tr("没有新增可靠关系；现有图谱保持不变。"),
      );
    } catch (e) {
      if (!disposed)
        message(
  ...
  function renderTools() {
    ...
    input.placeholder = tr("搜索知识点");
    input.setAttribute("aria-label", tr("搜索知识点"));
    input.value = query;
    ...
    select.setAttribute("aria-label", tr("知识点标签"));
    for (const t of ["", ...new Set(units.flatMap((u) => u.tags))]) {
      const o = el("option", "", t || tr("全部标签"));
      o.value = t;
      select.append(o);
    }
    select.value = tag;
  ...
  function render() {
    ...
    if (relation) inspectRelation(relation);
      ...
      detail.replaceChildren(
        el("p", "page-kicker", tr("INSPECT / 核查")),
        el("h2", "", tr("选择一个知识点")),
        el("p", "reader-muted", tr("点击节点或关系，查看解释并回到来源。")),
      );
    }
  }
  function draw() {
    ...
    if (!data.nodes.length) {
      empty(
        ...
        units.length
          ? tr("没有匹配且来源有效的知识点。")
          : tr("到 Reader 选择“沉淀为知识点”。"),
      );
      return;
    }
    const d3 = globalThis.d3;
    if (!d3) {
      message(canvas, tr("图谱组件加载失败，请刷新重试。"), true);
      return;
    }
  ...
  function renderSuggestions() {
    suggestions.replaceChildren(el("h2", "", tr("待核查的关系建议")));
    const pending = relations.filter((r) => r.status === "suggested");
    if (!pending.length) {
      suggestions.append(
        el("p", "reader-muted", tr("没有待确认建议。AI 不会自动连接你的知识点。")),
      );
      return;
    }
    for (const r of pending) {
      ...
      row.append(
        el(
          "h3",
          "",
          (a?.label || tr("来源缺失")) + " → " + (b?.label || tr("来源缺失")),
        ),
        el("small", "", r.type + tr(" · AI 建议")),
        el("p", "", r.reason),
      );
