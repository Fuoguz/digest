...
const nav = [
  ["/app/", "dashboard", tr("继续学习")],
  ["/app/library", "library", tr("资料库")],
  ["/app/review", "review", tr("复习")],
  ["/app/courses", "library", tr("课程")],
];
...
function sourceLabel(type) {
  return SOURCE_TYPES[type] || tr("资料");
}
...
function migrationNotice(migration) {
  ...
  return (
    th('<div class="notice"><span>已将旧版数据迁移为 ') +
    migration.migrated +
    th(
  ...
  const rows = filtered
    .map(
      (document) =>
        ...
        '" data-favorite="' +
        escapeHTML(document.id) +
        th('" aria-label="收藏">★</button></article>'),
    )
    .join("");
  return (
    th(
    ...
    th('" data-due-filter type="button">待复习</button></div>') +
    (filtered.length
      ? '<div class="document-table">' + rows + "</div>"
      : state.documents.length
        ? emptyState(
            tr("没有匹配的资料"),
            tr("调整关键词或筛选条件，查看资料库中的其他内容。"),
          ).replace(tr("导入第一份资料"), tr("导入新资料"))
        : emptyState(
            tr("资料库还没有内容"),
            tr("支持带文本层的 PDF、TXT、Markdown，也可以直接粘贴正文。"),
          )) +
    "</section>"
  );
}
